...
fn detect_vram_and_temp() -> (String, String) {
    ...
    let mut temp_str = "38.4°C".to_string();

    #[cfg(windows)]
    {
        if let Ok(output) = std::process::Command::new("nvidia-smi")
            .args(["--query-gpu=memory.total,temperature.gpu", "--format=csv,noheader,nounits"])
...
fn main() -> Result<(), slint::PlatformError> {
    ...
    timer.start(slint::TimerMode::Repeated, std::time::Duration::from_millis(60), move || {
        if let Some(ui) = ui_handle.upgrade() {
            ...
            // Every ~10 seconds: update real Disk and Temperature
            if tick % 160 == 0 {
                let (d_frac, d_disp) = get_disk_info();
                ui.set_disk_usage(d_frac);
                ui.set_disk_display(d_disp.into());

                let (_, cur_temp) = detect_vram_and_temp();
                ui.set_system_temp(cur_temp.into());
            }
        }
    });
    ...
    ui.on_send_message(move |user_input| {
        if let Some(ui) = ui_msg.upgrade() {
            ...
            let response = match input.to_lowercase().as_str() {
                ...
                "help" =>
                    "Available commands: 'status' (hardware health), 'clear' (reset session), 'export' (save session log).".to_string(),
                "temp" | "temperature" => {
